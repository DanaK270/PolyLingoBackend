  ...
  deletelanguage: async (req, res) => {
    ...
    try {
      ...
      if (lessonIds.length > 0) {
        ...
        const deleteLessonsResult = await Lesson.deleteMany({ _id: { $in: lessonIds } });
  ...
  deletelesson: async (req, res) => {
    ...
    try {
      ...
      if (updateLanguage.modifiedCount === 0) {
        return res.status(400).send({ message: "Lesson not found in any language fields" });
      }
  
      res.send({ message: "Lesson deleted and removed from language fields successfully" });
    } catch (err) {
      console.error(err); // Log the error for debugging purposes
      res.status(500).send({ message: "Error deleting lesson", error: err.message });
    }
  }
};
